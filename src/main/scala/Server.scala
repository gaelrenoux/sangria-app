...
import business.{CharacterRepo, SchemaDefinition}
...
import web.CorsSupport
import web.GraphQLRequestUnmarshaller._
...
object Server extends App with CorsSupport {
  ...
  def errorJson(error: Throwable): Json = error match {
    ...
    case NonFatal(e) => errorJson(e.getMessage)
    case e => throw e
  }
              ...
              QueryParser.parse(query) match {
                case Success(ast) =>
                  variables.map(parse) match {
                    case Some(Left(error)) => complete(BadRequest, errorJson(error))
                ...
                case Failure(error) => complete(BadRequest, errorJson(error))
              }
            }
        } ~
          post {
                ...
                query.map(QueryParser.parse(_)) match {
                  case Some(Success(ast)) =>
                    variablesStr.map(parse) match {
                      case Some(Left(error)) => complete(BadRequest, errorJson(error))
                      case Some(Right(json)) => executeGraphQL(ast, operationName, json, tracing.isDefined)
                      case None => executeGraphQL(ast, operationName, root.variables.json.getOption(body) getOrElse Json
                        .obj(), tracing.isDefined)
                    }
                  case Some(Failure(error)) => complete(BadRequest, errorJson(error))
                  case None => complete(BadRequest, errorJson("No query to execute"))
                }
              } ~
